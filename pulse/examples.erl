...
-module(examples).
-export([procA/0, example0/0, example1/0,  example2/0,
         example3/0, example4/0, example5/0, example6/0,
         example7/0, write_race/0, killing_time/0,
         receive_afterX/0, spawn_register_where/0]).
...
killing_time() ->
    ...
    spawn(fun () -> exit(Victim, die_die_die) end).
...
receive_afterX() ->
    ...
    _Proc2 = spawn(fun() -> exit(Proc1, foo), timer:sleep(10) end),
...
simple_reg(Reg) ->
    receive
        {'DOWN', _, _, Pid, _} ->
            simple_reg([X || X = {Pid1,_} <- Reg, Pid =/= Pid1]);
        {reg, From, Pid, Name} ->
            From ! {reg, reg_ok},
            erlang:monitor(process, Pid),
            simple_reg([{Pid,Name}|Reg]);
        {whereis, From, Name} ->
            case [Pid || {Pid,Name1} <- Reg, Name1 =:= Name] of
                [] -> From ! {reg, undefined};
                [Pid] -> From ! {reg, Pid}
            end,
            simple_reg(Reg);
        {stop, From} -> From ! {reg, stop_ok}
    end.
